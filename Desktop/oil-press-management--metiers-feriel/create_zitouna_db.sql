...
-- 10. Sequence FINANCE
-- ============================================================================
CREATE SEQUENCE SEQ_FINANCE START WITH 1 INCREMENT BY 1;
...
CREATE TABLE FINANCE (
    ...
    id_machine              NUMBER,
    idProduction            NUMBER,
    ctype                   VARCHAR2(30) NOT NULL,  -- REVENU ou DÉPENSE
    categorie              VARCHAR2(50) NOT NULL,  -- Vente, Achat, Maintenance, Salaire, etc.
    montant                NUMBER(12,2) NOT NULL,
    pourcentage_montant     NUMBER(5,2) DEFAULT 0,  -- Pourcentage du montant par rapport au total du type
    date_trans             DATE DEFAULT SYSDATE,
    description            VARCHAR2(500),
    user_id                NUMBER NOT NULL,
    
    CONSTRAINT FK_FIN_MACHINE FOREIGN KEY (id_machine) REFERENCES MACHINE(ID_MACHINE),
    CONSTRAINT FK_FIN_EMPLOYEE FOREIGN KEY (user_id) REFERENCES EMPLOYEES(USER_ID)
);

-- ============================================================================
-- 12. Donnees de test MACHINE
-- ============================================================================
...
-- ============================================================================
-- 15. Donnees de test EMPLOYEES
-- ============================================================================
INSERT INTO EMPLOYEES (USERNAME, EMAIL, PASSWORD_HASH, ROLE, STATE)
VALUES ('admin', 'admin@zitouna.com', 'admin123', 'ADMIN', 'ACTIVE');

INSERT INTO EMPLOYEES (USERNAME, EMAIL, PASSWORD_HASH, ROLE, STATE)
VALUES ('finance', 'finance@zitouna.com', 'finance123', 'FINANCE', 'ACTIVE');

COMMIT;

-- ============================================================================
-- 16. Donnees de test CLIENT
-- ============================================================================
...
-- ============================================================================
-- 17. Donnees de test FINANCE
-- ============================================================================
-- Total Revenus: 6800.00 | Total Dépenses: 1150.50
INSERT INTO FINANCE (id_transaction, user_id, ctype, categorie, montant, pourcentage_montant, description, date_trans, id_machine, idProduction)
VALUES (SEQ_FINANCE.NEXTVAL, 1, 'REVENU', 'Vente Huile', 1500.00, 22.06, 'Vente de 100L Huile Extra Vierge', SYSDATE, NULL, NULL);

INSERT INTO FINANCE (id_transaction, user_id, ctype, categorie, montant, pourcentage_montant, description, date_trans, id_machine, idProduction)
VALUES (SEQ_FINANCE.NEXTVAL, 1, 'DEPENSE', 'Maintenance', 350.50, 30.45, 'Entretien Presse Hydraulique A1', SYSDATE - 5, 1, NULL);

INSERT INTO FINANCE (id_transaction, user_id, ctype, categorie, montant, pourcentage_montant, description, date_trans, id_machine, idProduction)
VALUES (SEQ_FINANCE.NEXTVAL, 1, 'REVENU', 'Vente Huile', 2200.00, 32.35, 'Vente de 150L Huile Vierge', SYSDATE - 10, NULL, NULL);

INSERT INTO FINANCE (id_transaction, user_id, ctype, categorie, montant, pourcentage_montant, description, date_trans, id_machine, idProduction)
VALUES (SEQ_FINANCE.NEXTVAL, 1, 'DEPENSE', 'Achat Olives', 800.00, 69.55, 'Achat olives qualité premium', SYSDATE - 3, NULL, NULL);

INSERT INTO FINANCE (id_transaction, user_id, ctype, categorie, montant, pourcentage_montant, description, date_trans, id_machine, idProduction)
VALUES (SEQ_FINANCE.NEXTVAL, 1, 'REVENU', 'Vente Huile', 3100.00, 45.59, 'Vente de 200L Huile Lampante', SYSDATE - 15, NULL, NULL);

COMMIT;

-- ============================================================================
-- 18. Analysis Financiere - Revenus vs Depenses
-- ============================================================================
-- Vue pour l'analyse des revenus et dépenses par catégorie avec pourcentages
CREATE OR REPLACE VIEW VW_FINANCE_SUMMARY AS
SELECT 
    ctype AS Type_Transaction,
    categorie AS Categorie,
    COUNT(*) AS Nombre_Transactions,
    SUM(montant) AS Montant_Total,
    ROUND(SUM(montant) / (SELECT SUM(montant) FROM FINANCE WHERE ctype = F.ctype) * 100, 2) AS Pourcentage_par_Type,
    ROUND(SUM(montant) / (SELECT SUM(montant) FROM FINANCE) * 100, 2) AS Pourcentage_Total
FROM FINANCE F
GROUP BY ctype, categorie
ORDER BY ctype DESC, Montant_Total DESC;

-- Vue pour la répartition Revenus vs Dépenses
CREATE OR REPLACE VIEW VW_FINANCE_REPARTITION AS
SELECT 
    ctype AS Type,
    SUM(montant) AS Montant_Total,
    COUNT(*) AS Nombre_Transactions,
    ROUND(SUM(montant) / (SELECT SUM(montant) FROM FINANCE) * 100, 2) AS Pourcentage_Repartition,
