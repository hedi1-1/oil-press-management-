...
**Un module applicatif robuste pour la supervision, la maintenance et l'analyse de votre parc de machines.**
...
Ce fichier documente exhaustivement les composants de l'interface utilisateur (widgets) et les fonctions C++ utilisés dans l'application de gestion des machines.

## 🖥️ Documentation de l'Interface (Widgets)
...
### 3. 🗂️ Gestion du Parc Machines (Tableau Principal)
...
- `tableMachines` : Vue tabulaire (`QTableView`) listant toutes les machines avec leurs attributs. Supporte le double-clic pour la modification.
...
### 5. 🛠️ Maintenance & Suivi (Todo List)
...
- `btnGenererStats_machine` : Lance la génération du graphique.
...
- `progressConfianceIA_machine` : Barre de progression indiquant le niveau de certitude (0-100%).
- `btnAppliquerDecision_machine` : Permet d'exécuter automatiquement la recommandation suggérée.
- `btnIgnorerDecision_machine` : Ferme ou ignore la suggestion actuelle.

---

## ⚙️ Implémentation Logicielle - Module Qt (C++)

Ce module gère le parc machines d'une huilerie avec une interface Qt reliée à une base Oracle (`QODBC`). Il inclut un CRUD complet, filtres avancés, export multi-format, suivi maintenance, etc.

### 🟢 1) CREATE - Ajouter une machine
Intègre vérification d'unicité, dialogue intelligent et auto-suggestions :
- `on_pushButton_enregistrer_machine_clicked`
- `collectMachineFormData`
- `isMachineNameAvailable`
- `buildAvailableNameSuggestions`
- `showRequiredNameDialog`
- `showDuplicateNameDialog`
- `insertMachine`
- `saveMachineWithName`

### 🔵 2) READ - Afficher/consulter les données
Chargement des machines, jointure employé et coloration sémantique :
- `chargerMachines`
- `setupMachineTable`
- `setupEmployeesTable`
- `chargerEmployees`
- `machineEmployeeColumnName`
- `machineHasEmployeeColumn`
- `showMachineCard`

### 🟠 3) UPDATE - Modifier une machine
Mise à jour SQL des champs et horodatage modifiés via formulaire modal :
- `on_tableMachines_machine_doubleClicked`
- `on_btnModifierMachine_machine_clicked`

### 🔴 4) DELETE - Supprimer une machine
Exécution `DELETE SQL` avec confirmation utilisateur et rafraîchissement auto :
- `on_btnSupprimerMachine_machine_clicked`

---

## 🔧 Autres Fonctions Implémentées

### 🔍 Recherche, Filtres et Tri
Filtres multi-critères et tri avancé:
- `rechercherMachines`
- `appliquerFiltres`
- `on_btnFiltrer_clicked`
- `refreshResponsableFilterOptions`

### 🔌 Changement d'État ON/OFF
Bascule persistante immédiate :
- lambda connectée à `btnToggleOnOff_machine`

### 📑 Export Multi-format des Fiches Machine
Génération PDF, ODT, CSV, TXT de fiches structurées :
- `afficherDialogExport`
- `exporterPDF`
- `ExportDialog::setupUi`
- `ExportDialog::getSelectedFormat`

### ⏱️ Historique des États (Temps ON/OFF/VEILLE)
...
### 🛠️ Todo Maintenance
Gestion de la priorisation des tâches :
- `setupTodoList`
- `addTodoItem`
- `refreshTodoList`
- `createTodoItemWidget`
- `onTodoCheckToggled`
- `showAddTodoDialog`

### 🧭 Navigation et Ergonomie
- `NavigationBar::addTab` / `setActiveTab` / `onTabButtonClicked`
- `onNavigationTabClicked`
- `updateDateTime`
- `eventFilter`
- `on_btnReinitialiser_machine_clicked`

### 🗄️ Gestion de la Connexion BD
Singleton de connexion Oracle :
- `ConnectionMachine::getInstance`
- `ConnectionMachine::createconnect`
- `ConnectionMachine::getDatabase`

---

### ✨ Résumé Global
Le code actuel couvre :
- CRUD machine complet.
- Contrôle de cohérence (nom obligatoire + unicité).
- Enrichissements métier (responsable, historiques, export, maintenance).
- Interface opérationnelle avec mise à jour en temps réel.
