...
void TransactionTab::initializeUI()
{
    ...
    cbType->addItems({"Revenu", "Dépense"});
    row1->addWidget(cbType);
    formLayout->addLayout(row1);

    // Row 2
    QHBoxLayout *row2 = new QHBoxLayout();
    row2->addWidget(new QLabel("Catégorie:"));
    ...
    btnClear = new QPushButton("Réinitialiser");
    ...
    tableTransaction->setHorizontalHeaderLabels({"ID", "Type", "Catégorie", "Montant", "Date", "Client", "Description", "Actions"});
...
void SearchTab::initializeUI()
{
    ...
    cbSearchType->addItems({"Client", "Catégorie", "Montant", "Date"});
    ...
    // ===== Résultats =====
    QGroupBox *resultGroup = new QGroupBox("Résultats", this);
    QVBoxLayout *resultLayout = new QVBoxLayout(resultGroup);
    tableResult = new QTableWidget();
    tableResult->setColumnCount(7);
    tableResult->setHorizontalHeaderLabels({"ID", "Type", "Catégorie", "Montant", "Date", "Client", "Description"});
...
{
    QGroupBox *summaryGroup = new QGroupBox("📊 Résumé Financier", this);
    ...
    QLabel *lblRevenuTitle = new QLabel("💰 Total Revenus");
    lblRevenuTitle->setStyleSheet("font-weight: bold; font-size: 11pt; color: #1A3C2F;");
    ...
    QLabel *lblDepenseTitle = new QLabel("💸 Total Dépenses");
    lblDepenseTitle->setStyleSheet("font-weight: bold; font-size: 11pt; color: #1A3C2F;");
    ...
    QLabel *lblBeneficeTitle = new QLabel("🎯 Bénéfice Net");
    lblBeneficeTitle->setStyleSheet("font-weight: bold; font-size: 11pt; color: #1A3C2F;");
    lblBenefice = new QLabel("0.00 DT");
    lblBenefice->setStyleSheet("font-weight: bold; font-size: 16pt; color: #1A3C2F;");
    ...
    benCard->setStyleSheet("background-color: #f5f9f8; border: 2px solid #1A3C2F; border-radius: 8px; padding: 15px;");
...
{
    QGroupBox *controlGroup = new QGroupBox("⚙️ Configuration Graphique", this);
    ...
    cbChartType->addItems({"Camembert (Revenus/Dépenses)", "Histogramme Mensuel", "Courbe Tendance", "Comparatif Catégories"});
    cbChartType->setMinimumWidth(250);

    btnGenerate = new QPushButton("🔄 Générer Graphique");
...
{
    QGroupBox *chartGroup = new QGroupBox("📈 Visualisation", this);
    ...
    QLabel *placeholderLabel = new QLabel("Sélectionnez un type et cliquez sur 'Générer Graphique'");
...
void JournalTab::initializeUI()
{
    ...
    cbTypeFilter->addItems({"Tous", "Revenu", "Dépense"});
    filterRow2->addWidget(cbTypeFilter);
    filterRow2->addWidget(new QLabel("Catégorie:"));
    ...
    tableJournal->setHorizontalHeaderLabels({"Date", "Type", "Catégorie", "Montant", "Client", "Description", "Statut"});
...
void BudgetTab::initializeUI()
{
    ...
    // ===== Budget =====
    QGroupBox *budgetGroup = new QGroupBox("Définir le Budget", this);
    ...
    cbMonth->addItems({"Janvier", "Février", "Mars", "Avril", "Mai", "Juin",
    ...
    // ===== Résultat Comparatif =====
    QGroupBox *resultGroup = new QGroupBox("Comparaison avec Dépenses Réelles", this);
    QVBoxLayout *resultLayout = new QVBoxLayout(resultGroup);
    ...
    // Dépense Réelle
    QVBoxLayout *depLayout = new QVBoxLayout();
    depLayout->addWidget(new QLabel("Dépense Réelle (DT):"));
    ...
    ecartLayout->addWidget(new QLabel("Écart (DT):"));
...
void AdvancedTab::initializeUI()
{
    ...
    // =========================================================================
    QGroupBox *rentGroup = new QGroupBox("Rentabilité", contentWidget);
    ...
    cbMonthRent->addItems({"Janvier", "Février", "Mars", "Avril", "Mai", "Juin",
                           "Juillet", "Août", "Septembre", "Octobre", "Novembre", "Décembre"});
    cbMonthRent->setMinimumWidth(150);
    cbMonthRent->setMaximumWidth(250);
    rentLayout->addWidget(cbMonthRent);

    btnCalcRent = new QPushButton("Calculer Rentabilité");
    ...
    rentLayout->addWidget(new QLabel("Résultat:"));
    lblRent = new QLabel("N/A");
    lblRent->setObjectName("lblRent");
    lblRent->setStyleSheet("font-weight: bold; color: #1A3C2F; font-size: 14pt; margin-left: 10px;");
    ...
    // =========================================================================
    QGroupBox *anomalyGroup = new QGroupBox("Détection Dépenses Anormales", contentWidget);
    ...
    btnDetect = new QPushButton("Détecter");
    ...
    tableAnomaly->setHorizontalHeaderLabels({"Date", "Catégorie", "Montant", "Écart %", "Alerte"});
    ...
    // 3. PRÉVISION SECTION
    // =========================================================================
    QGroupBox *predictGroup = new QGroupBox("Prévision", contentWidget);
    ...
    predictControls->addWidget(new QLabel("Période:"));
    ...
    btnPredict = new QPushButton("Prédire");
    ...
    QLabel *lblChartPlaceholder = new QLabel("Zone Graphique Prévisionnelle", chartPredict);
    ...
    cbRank->addItems({"Chiffre d'Affaires", "Fréquence", "Récemment Actif"});
    ...
    // =========================================================================
    QGroupBox *simGroup = new QGroupBox("Simulation Financière", contentWidget);
    ...
    // Row 1
    simGrid->addWidget(new QLabel("Ventes (Qtés):"), 1, 0);
    ...
    simResultLayout->addWidget(new QLabel("Résultat de simulation :"));
    lblResult = new QLabel("N/A");
    lblResult->setObjectName("lblResult");
    lblResult->setStyleSheet("font-weight: bold; font-size: 16px; color: #1A3C2F; padding: 6px 15px; border: 2px solid #1A3C2F; border-radius: 6px; background-color: #e6fffa;");
...
void ExportTab::initializeUI()
{
    ...
    // ===== Export =====
    QGroupBox *exportGroup = new QGroupBox("Paramètres d'Export", this);
...
void MainWindow::initializeUI()
{
    ...
    headerWidget->setStyleSheet(
        ...
        "stop:0 #2D5F4D, stop:0.5 #3A7A62, stop:1 #2D5F4D); "
        "border-bottom: 4px solid #D4A852; "
        "}"
    );
...
void MainWindow::applyStyles()
{
    ...
    QString stylesheet = R"(
        ...
        QLineEdit, QSpinBox, QDoubleSpinBox, QDateEdit, QComboBox {
            ...
            font-family: 'Segoe UI', Arial, sans-serif;
            min-height: 20px;
            selection-background-color: #2D5F4D;
        }
