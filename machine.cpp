...
// ============================================================================
// Arduino Machine Dialog
// ============================================================================

ArduinoMachineDialog::ArduinoMachineDialog(const QString &machineId,
                                           const QString &machineName,
                                           const QString &machineType,
                                           bool isFrench,
                                           bool isDarkMode,
                                           QWidget *parent)
    : QDialog(parent), m_machineId(machineId), m_machineName(machineName),
      m_machineType(machineType), m_isFrench(isFrench), m_isDarkMode(isDarkMode) {
  setAttribute(Qt::WA_DeleteOnClose);
  setWindowTitle(m_isFrench ? "Arduino - Controle machine" : "Arduino - Machine control");
  resize(620, 420);
  ...
  connect(m_connectButton, &QPushButton::clicked,
          this, &ArduinoMachineDialog::toggleConnection);
  connect(m_refreshPortsButton, &QPushButton::clicked,
          this, &ArduinoMachineDialog::refreshPortList);
  connect(m_serial, &QSerialPort::readyRead,
          this, &ArduinoMachineDialog::readSerialData);
  connect(m_thresholdCombo, QOverload<int>::of(&QComboBox::currentIndexChanged),
          this, &ArduinoMachineDialog::onThresholdModeChanged);
}

ArduinoMachineDialog::~ArduinoMachineDialog() {
  if (m_serial && m_serial->isOpen()) {
    m_serial->close();
  }
}

void ArduinoMachineDialog::buildUi() {
  auto *root = new QVBoxLayout(this);
  root->setContentsMargins(16, 16, 16, 16);
  root->setSpacing(12);

  m_headerLabel = new QLabel(formatHeader(), this);
  m_headerLabel->setWordWrap(true);
  root->addWidget(m_headerLabel);

  m_statusLabel = new QLabel(
      m_isFrench ? "Statut: deconnecte" : "Status: disconnected", this);
  root->addWidget(m_statusLabel);

  auto *connectionGroup = new QGroupBox(
      m_isFrench ? "Connexion Arduino" : "Arduino connection", this);
  auto *connectionLayout = new QFormLayout(connectionGroup);
  connectionLayout->setLabelAlignment(Qt::AlignLeft);

  m_portCombo = new QComboBox(connectionGroup);
  m_refreshPortsButton = new QPushButton(
      m_isFrench ? "Actualiser ports" : "Refresh ports", connectionGroup);
  auto *portRow = new QWidget(connectionGroup);
  ...
  connectionLayout->addRow(m_isFrench ? "Port COM:" : "COM port:", portRow);
  ...
  connectionLayout->addRow(m_isFrench ? "Vitesse:" : "Baud rate:", m_baudCombo);

  m_thresholdCombo = new QComboBox(connectionGroup);
  m_thresholdCombo->addItem(m_isFrench
                                ? "Hypothese A - Standard"
                                : "Hypothesis A - Standard");
  m_thresholdCombo->addItem(m_isFrench
                                ? "Hypothese B - Environnement chaud"
                                : "Hypothesis B - Hot environment");
  m_thresholdCombo->addItem(m_isFrench
                                ? "Hypothese C - Adaptatif par type"
                                : "Hypothesis C - Adaptive by type");
  m_thresholdCombo->setCurrentIndex(static_cast<int>(m_thresholdMode));
  connectionLayout->addRow(m_isFrench ? "Profil seuils:" : "Threshold profile:",
                           m_thresholdCombo);

  m_connectButton = new QPushButton(
      m_isFrench ? "Connecter" : "Connect", connectionGroup);
  connectionLayout->addRow(QString(), m_connectButton);
  root->addWidget(connectionGroup);

  auto *telemetryGroup = new QGroupBox(
      m_isFrench ? "Mesures en temps reel" : "Live measurements", this);
  auto *telemetryLayout = new QGridLayout(telemetryGroup);
  telemetryLayout->setContentsMargins(12, 12, 12, 12);
  telemetryLayout->setHorizontalSpacing(18);
  telemetryLayout->setVerticalSpacing(10);

  auto *tempLabel = new QLabel(m_isFrench ? "Temperature:" : "Temperature:", telemetryGroup);
  m_tempValueLabel = new QLabel("--.- C", telemetryGroup);
  auto *humidityLabel = new QLabel(m_isFrench ? "Humidite:" : "Humidity:", telemetryGroup);
  m_humidityValueLabel = new QLabel("-- %", telemetryGroup);
  auto *levelLabel = new QLabel(m_isFrench ? "Niveau:" : "Level:", telemetryGroup);
  m_levelValueLabel = new QLabel("--", telemetryGroup);
  auto *fanLabel = new QLabel(m_isFrench ? "Ventilateur:" : "Fan:", telemetryGroup);
  m_fanValueLabel = new QLabel("OFF", telemetryGroup);
  m_fanValueLabel->setAlignment(Qt::AlignCenter);
  m_fanValueLabel->setMinimumWidth(70);

  telemetryLayout->addWidget(tempLabel, 0, 0);
  telemetryLayout->addWidget(m_tempValueLabel, 0, 1);
  telemetryLayout->addWidget(humidityLabel, 1, 0);
  telemetryLayout->addWidget(m_humidityValueLabel, 1, 1);
  telemetryLayout->addWidget(levelLabel, 0, 2);
  telemetryLayout->addWidget(m_levelValueLabel, 0, 3);
  telemetryLayout->addWidget(fanLabel, 1, 2);
  telemetryLayout->addWidget(m_fanValueLabel, 1, 3);

  root->addWidget(telemetryGroup);
}

void ArduinoMachineDialog::applyTheme() {
  const QString headerStyle = m_isDarkMode
      ? "QLabel { color: #EAF0F8; font-size: 14px; font-weight: 700; }"
      : "QLabel { color: #1B2F2A; font-size: 14px; font-weight: 700; }";
  const QString statusStyle = m_isDarkMode
      ? "QLabel { color: #9EB0C4; font-size: 12px; }"
      : "QLabel { color: #4A5A52; font-size: 12px; }";

  if (m_headerLabel) {
    m_headerLabel->setStyleSheet(headerStyle);
  }
  if (m_statusLabel) {
...
void ArduinoMachineDialog::setConnectionState(bool connected) {
  if (m_connectButton) {
    m_connectButton->setText(connected
                                 ? (m_isFrench ? "Deconnecter" : "Disconnect")
                                 : (m_isFrench ? "Connecter" : "Connect"));
  }
  if (m_statusLabel) {
    m_statusLabel->setText(connected
                               ? (m_isFrench ? "Statut: connecte" : "Status: connected")
                               : (m_isFrench ? "Statut: deconnecte" : "Status: disconnected"));
  }
}

void ArduinoMachineDialog::toggleConnection() {
  if (!m_serial) {
    return;
  }
  ...
  const QString portName = m_portCombo ? m_portCombo->currentText().trimmed() : QString();
  if (portName.isEmpty()) {
    if (m_statusLabel) {
      m_statusLabel->setText(m_isFrench ? "Statut: port non selectionne" : "Status: port not selected");
    }
    return;
  }

  const int baudRate = m_baudCombo ? m_baudCombo->currentText().toInt() : 9600;
  ...
  if (!m_serial->open(QIODevice::ReadOnly)) {
    if (m_statusLabel) {
      m_statusLabel->setText(m_isFrench ? "Statut: echec connexion" : "Status: connection failed");
    }
    return;
  }
...
void ArduinoMachineDialog::refreshPortList() {
  if (!m_portCombo) {
    return;
  }
...
void ArduinoMachineDialog::readSerialData() {
  if (!m_serial) {
    return;
  }
  ...
  while (newlineIndex >= 0) {
    const QByteArray lineData = m_serialBuffer.left(newlineIndex);
    m_serialBuffer.remove(0, newlineIndex + 1);
    const QString line = QString::fromUtf8(lineData).trimmed();
    if (!line.isEmpty()) {
      handleSerialLine(line);
    }
    newlineIndex = m_serialBuffer.indexOf('\n');
  }
}

void ArduinoMachineDialog::onThresholdModeChanged(int index) {
  m_thresholdMode = static_cast<ArduinoThresholdMode>(index);
}

void ArduinoMachineDialog::handleSerialLine(const QString &line) {
  if (line.startsWith("Erreur", Qt::CaseInsensitive)) {
    if (m_statusLabel) {
      m_statusLabel->setText(m_isFrench ? "Statut: erreur capteur" : "Status: sensor error");
    }
    return;
  }

  QRegularExpression re(
      "Temp:\\s*([0-9]+(?:\\.[0-9]+)?)\\s*C\\s*\\|\\s*Humidite:\\s*"
      "([0-9]+(?:\\.[0-9]+)?)\\s*%\\s*\\|\\s*Niveau:\\s*([^|]+)\\|\\s*Ventilateur:\\s*(ON|OFF)",
  ...
  const QString level = match.captured(3).trimmed();
  const QString fanState = match.captured(4).trimmed().toUpper();

  updateTelemetry(temperature, humidity, level, fanState);
}

void ArduinoMachineDialog::updateTelemetry(double temperature,
                                           double humidity,
                                           const QString &level,
                                           const QString &fanState) {
  if (m_tempValueLabel) {
    m_tempValueLabel->setText(QString::number(temperature, 'f', 1) + " C");
  }
  if (m_humidityValueLabel) {
    m_humidityValueLabel->setText(QString::number(humidity, 'f', 0) + " %");
  }
  if (m_levelValueLabel) {
    m_levelValueLabel->setText(level);
  }
  if (m_fanValueLabel) {
    m_fanValueLabel->setText(fanState);
    const QString fanStyle = (fanState == "ON")
        ? "QLabel { background-color: #2E7D32; color: white; border-radius: 6px; padding: 4px 8px; font-weight: 700; }"
        : "QLabel { background-color: #B0BEC5; color: #263238; border-radius: 6px; padding: 4px 8px; font-weight: 700; }";
    m_fanValueLabel->setStyleSheet(fanStyle);
  }

  const ArduinoThresholdProfile profile = resolveThresholdProfile();

  QString etatFonctionnement = "Normal";
  QString typeAlerte = "Aucune";
  QString niveauCriticite = "Faible";
  if (temperature >= profile.panne) {
    etatFonctionnement = "Panne";
    typeAlerte = "Panne";
    niveauCriticite = "Critique";
  } else if (temperature >= profile.alert) {
    etatFonctionnement = "Alerte";
    typeAlerte = "Surcharge";
    niveauCriticite = "Élevé";
  }

  const int score = computeHealthScore(temperature, profile, etatFonctionnement);
  const QString fanDb = (fanState == "ON") ? "on" : "off";

  QSqlQuery query(ConnectionMachine::getInstance().getDatabase());
  query.prepare(
      "UPDATE MACHINE SET TEMPERATURE_ACTUELLE = :temp, "
      "REFROIDISSEMENT = :fan, ETAT_FONCTIONNEMENT = :etat, "
      "TYPE_ALERTE = :alerte, NIVEAU_CRITICITE = :criticite, "
      "SCORE_SANTE = :score, DATE_MISE_A_JOUR = SYSTIMESTAMP "
      "WHERE ID_MACHINE = :id");
  query.bindValue(":temp", temperature);
  query.bindValue(":fan", fanDb);
  query.bindValue(":etat", etatFonctionnement);
  query.bindValue(":alerte", typeAlerte);
  query.bindValue(":criticite", niveauCriticite);
  query.bindValue(":score", score);
  query.bindValue(":id", m_machineId);

  if (query.exec()) {
    emit telemetryApplied(m_machineId);
  } else if (m_statusLabel) {
    m_statusLabel->setText(m_isFrench ? "Statut: erreur mise a jour" : "Status: update error");
  }
}

ArduinoThresholdProfile ArduinoMachineDialog::resolveThresholdProfile() const {
  switch (m_thresholdMode) {
    case ArduinoThresholdMode::HypothesisA:
      return {35.0, 50.0, 65.0};
    case ArduinoThresholdMode::HypothesisB:
      return {45.0, 70.0, 85.0};
    case ArduinoThresholdMode::HypothesisC:
      return loadProfileFromDatabase();
  }
  return {35.0, 50.0, 65.0};
}

ArduinoThresholdProfile ArduinoMachineDialog::loadProfileFromDatabase() const {
  QSqlQuery query(ConnectionMachine::getInstance().getDatabase());
  query.prepare(
      "SELECT FAN_ON_TEMP, ALERT_TEMP, PANNE_TEMP "
      "FROM MACHINE_THRESHOLDS WHERE UPPER(TYPE_MACHINE) = UPPER(:type)");
  query.bindValue(":type", m_machineType);

  if (!query.exec()) {
    qDebug() << "[Arduino] Impossible de charger les seuils:" << query.lastError().text();
    return {35.0, 50.0, 65.0};
  }

  if (query.next()) {
    ArduinoThresholdProfile profile;
    profile.fanOn = query.value(0).toDouble();
    profile.alert = query.value(1).toDouble();
    profile.panne = query.value(2).toDouble();
    if (profile.alert <= 0.0 || profile.panne <= 0.0) {
      return {35.0, 50.0, 65.0};
    }
    return profile;
  }
...
QString ArduinoMachineDialog::formatHeader() const {
  const QString name = m_machineName.trimmed().isEmpty() ? "Machine" : m_machineName.trimmed();
  const QString type = m_machineType.trimmed().isEmpty() ? "-" : m_machineType.trimmed();
  if (m_isFrench) {
    return QString("Machine: %1 | ID: %2 | Type: %3").arg(name, m_machineId, type);
  }
  return QString("Machine: %1 | ID: %2 | Type: %3").arg(name, m_machineId, type);
}

int ArduinoMachineDialog::computeHealthScore(double temperature,
                                             const ArduinoThresholdProfile &profile,
                                             const QString &etatFonctionnement) const {
  if (etatFonctionnement == "Panne") {
    return 20;
  }
  if (etatFonctionnement == "Alerte") {
    return 60;
  }
  if (temperature >= profile.fanOn) {
    return 85;
  }
  return 95;
}
...
machine::machine(QWidget *parent)
  ...
  m_btnVoiceAssistant = findChild<QPushButton *>("btnVoiceAssistant_machine");
  if (m_btnVoiceAssistant) {
    m_assistant->setTalkButton(m_btnVoiceAssistant);
    connect(m_btnVoiceAssistant, &QPushButton::clicked,
...
void machine::applyTheme() {
  ...
  if (m_btnVoiceAssistant) {
    m_btnVoiceAssistant->setStyleSheet(iconButtonStyle);
  }
  ...
  ui->lblDateTime_machine->setStyleSheet(m_isDarkMode ? darkDateStyle
    ...
    ui->lblOnlineStatus_machine->setStyleSheet(m_isDarkMode ? darkOnlineStyle
      ...
      if (m_arduinoTitleLabel) {
        const QString titleStyle = m_isDarkMode
            ? "QLabel { color: #EAF0F8; font-size: 13px; font-weight: 700; }"
            : "QLabel { color: #1B4D3E; font-size: 13px; font-weight: 700; }";
        m_arduinoTitleLabel->setStyleSheet(titleStyle);
      }
      if (m_arduinoEmptyLabel) {
        const QString emptyStyle = m_isDarkMode
            ? "QLabel { color: #B8C7D8; font-size: 12px; font-weight: 600; }"
            : "QLabel { color: #6B7D75; font-size: 12px; font-weight: 600; }";
        m_arduinoEmptyLabel->setStyleSheet(emptyStyle);
      }
}
...
void machine::applyLanguage() {
  ...
  if (m_btnVoiceAssistant) {
    m_btnVoiceAssistant->setToolTip(
    ...
    if (m_arduinoTitleLabel) {
      m_arduinoTitleLabel->setText(
          m_isFrench ? "Machines eligibles Arduino" : "Arduino eligible machines");
    }
    if (m_btnArduinoRefresh) {
      m_btnArduinoRefresh->setText(m_isFrench ? "Actualiser" : "Refresh");
    }
    if (m_arduinoEmptyLabel && m_arduinoButtons.isEmpty()) {
      m_arduinoEmptyLabel->setText(
          m_isFrench ? "Aucune machine eligible." : "No eligible machine.");
    }
...
void machine::setupArduinoModule() {
  ...
  m_arduinoPanel = new QWidget(this);
  auto *panelLayout = new QVBoxLayout(m_arduinoPanel);
  panelLayout->setContentsMargins(6, 6, 6, 6);
  panelLayout->setSpacing(10);

  auto *headerLayout = new QHBoxLayout();
    m_arduinoTitleLabel = new QLabel(
      m_isFrench ? "Machines eligibles Arduino" : "Arduino eligible machines",
      m_arduinoPanel);
  m_btnArduinoRefresh = new QPushButton(
      m_isFrench ? "Actualiser" : "Refresh", m_arduinoPanel);
    headerLayout->addWidget(m_arduinoTitleLabel);
  headerLayout->addStretch();
  headerLayout->addWidget(m_btnArduinoRefresh);
  panelLayout->addLayout(headerLayout);

  m_arduinoScroll = new QScrollArea(m_arduinoPanel);
  m_arduinoScroll->setWidgetResizable(true);
  m_arduinoScroll->setFrameShape(QFrame::NoFrame);
  auto *scrollContent = new QWidget(m_arduinoScroll);
  m_arduinoGrid = new QGridLayout(scrollContent);
  m_arduinoGrid->setContentsMargins(6, 6, 6, 6);
  m_arduinoGrid->setHorizontalSpacing(12);
  m_arduinoGrid->setVerticalSpacing(12);
  scrollContent->setLayout(m_arduinoGrid);
  m_arduinoScroll->setWidget(scrollContent);
  panelLayout->addWidget(m_arduinoScroll, 1);

  ui->layoutArduinoPlaceholder->addWidget(m_arduinoPanel);

  connect(m_btnArduinoRefresh, &QPushButton::clicked,
          this, &machine::refreshArduinoEligibleMachines);

  m_arduinoRefreshTimer = new QTimer(this);
  connect(m_arduinoRefreshTimer, &QTimer::timeout,
          this, &machine::refreshArduinoEligibleMachines);
  m_arduinoRefreshTimer->start(15000);

  refreshArduinoEligibleMachines();
  applyTheme();
  applyLanguage();
}

void machine::clearArduinoMachineButtons() {
  if (!m_arduinoGrid) {
    return;
  }

  QLayoutItem *child = nullptr;
  while ((child = m_arduinoGrid->takeAt(0)) != nullptr) {
    if (child->widget()) {
      delete child->widget();
    }
    delete child;
  }
  m_arduinoButtons.clear();
  m_arduinoEmptyLabel = nullptr;
}

void machine::refreshArduinoEligibleMachines() {
  if (!m_arduinoGrid) {
    return;
  }

  clearArduinoMachineButtons();

  QSqlQuery query(ConnectionMachine::getInstance().getDatabase());
  query.prepare(
      "SELECT ID_MACHINE, NOM_MACHINE, TYPE_MACHINE "
      "FROM MACHINE "
      "WHERE UPPER(ETAT_MARCHE) = 'ON' "
      "AND UPPER(ETAT_FONCTIONNEMENT) = 'NORMAL' "
      "AND UPPER(TYPE_ALERTE) = 'AUCUNE' "
      "AND UPPER(NIVEAU_CRITICITE) = 'FAIBLE' "
      "ORDER BY NOM_MACHINE ASC");

  if (!query.exec()) {
    m_arduinoEmptyLabel = new QLabel(
        m_isFrench ? "Erreur de chargement des machines." : "Unable to load machines.",
        m_arduinoPanel);
    m_arduinoGrid->addWidget(m_arduinoEmptyLabel, 0, 0);
    return;
  }
  ...
  while (query.next()) {
    ...
    const QString type = query.value(2).toString().trimmed();

    QString buttonText = name.isEmpty() ? QString("Machine %1").arg(id) : name;
    auto *button = new QPushButton(buttonText, m_arduinoPanel);
    button->setProperty("machineId", id);
    button->setProperty("machineType", type);
    button->setToolTip(QString("ID: %1\nType: %2").arg(id, type));
    button->setStyleSheet(
        "QPushButton { background-color: #1B4D3E; color: white; padding: 8px 14px; "
        "border-radius: 6px; font-weight: 600; } "
        "QPushButton:hover { background-color: #163E31; }");

    connect(button, &QPushButton::clicked, this, [this, id]() {
      openArduinoMachineDialog(id);
    });

    m_arduinoGrid->addWidget(button, row, col);
    m_arduinoButtons.append(button);
    ++count;
  ...
  if (count == 0) {
    m_arduinoEmptyLabel = new QLabel(
        m_isFrench ? "Aucune machine eligible." : "No eligible machine.",
        m_arduinoPanel);
    m_arduinoEmptyLabel->setAlignment(Qt::AlignCenter);
    m_arduinoEmptyLabel->setStyleSheet(
      m_isDarkMode
        ? "QLabel { color: #B8C7D8; font-size: 12px; font-weight: 600; }"
        : "QLabel { color: #6B7D75; font-size: 12px; font-weight: 600; }");
    m_arduinoGrid->addWidget(m_arduinoEmptyLabel, 0, 0, 1, columns);
  }
}

void machine::openArduinoMachineDialog(const QString &machineId) {
  if (machineId.trimmed().isEmpty()) {
    return;
  }
  ...
  QSqlQuery query(ConnectionMachine::getInstance().getDatabase());
  query.prepare(
      "SELECT NOM_MACHINE, TYPE_MACHINE FROM MACHINE WHERE ID_MACHINE = :id");
  query.bindValue(":id", machineId);
  if (query.exec() && query.next()) {
    machineName = query.value(0).toString();
    machineType = query.value(1).toString();
  } else {
    for (const MachineData &m : m_allMachines) {
      if (m.id == machineId) {
        machineName = m.nom;
  ...
  auto *dialog = new ArduinoMachineDialog(
      machineId, machineName, machineType, m_isFrench, m_isDarkMode, this);
  connect(dialog, &ArduinoMachineDialog::telemetryApplied,
          this, &machine::onArduinoTelemetryApplied);
  dialog->show();
}

void machine::onArduinoTelemetryApplied(const QString &machineId) {
  Q_UNUSED(machineId);
  chargerMachines();
}
...
void machine::exporterPDF() {
  ...
  QString html =
      QString::fromUtf8(
          ...
          "    <span class='field-value'>%6</span></div>"
          "  <div class='field'>"
          "\xE2" "\x9A" "\xA1 <span class='field-label'>Charge "
          "(%%)</span>&nbsp;&nbsp;&nbsp;"
          "    <span class='field-value'>%7</span></div>"
          ...
          "\xF0" "\x9F" "\x8F" "\xAD  Syst\xC3\xA8me de "
...
void machine::chargerHistorique() {
  ...
  ui->btnFermerHistorique->setText(
...
void machine::showMachineCard() {
  ...
  addField(1, 1, QString::fromUtf8("Temp\xC3\xa9rature:"),
           temperature + QString::fromUtf8(" \xC2\xB0") + "C");
...
void machine::updateMachineCounts() {
  ...
  ui->lblMachinesNormales_machine->setText(
      QString("<html><head/><body>"
              ...
              "<span style=\" font-size:14pt; font-weight:700; color:#2E7D32;\">"
              "\xE2" "\x9C" "\x93</span>"
  ...
  ui->lblMachinesPanne_machine->setText(
      QString("<html><head/><body>"
              "<p style=\"margin:0; padding:0;\">"
              "<span style=\" font-size:16pt; font-weight:700; color:#D32F2F;\">"
              "\xE2" "\x9C" "\x97</span>"
